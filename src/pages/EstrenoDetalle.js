...
const EstrenoDetalle = () => {
    ...
    useEffect(() => {
        const fetchEstrenoDetalle = async () => {
            ...
            try {
                ...
                if (data && data.data && data.imageBaseUrl) {
                    ...
                    const estrenoDetalle = data.data.find(movie => movie.id.toString() === id);

                    if (estrenoDetalle) {
                        setMovie(estrenoDetalle);
                    } else {
                                        ...
                                        {actors.slice(index, index + 4).map(actor => (
                                            <div key={actor.id} className="col-3">
                                                <div className="text-center">
                                                    ...
                                                    />
                                                    <h5 style={{ fontSize: '1rem', marginTop: '10px' }}>{actor.name}</h5>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
